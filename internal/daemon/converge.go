...
func (manager *Manager) convergeOnePath(ctx context.Context, codebase model.Codebase, relativePath string, snapshot *merkle.Snapshot, admission *admissionState) bool {
	...
	if fileResult.Removed {
		if rmErr := manager.semantic.Reindex(ctx, root, nil, semantic.RemovePaths([]string{relativePath}), nil, nil, semantic.StoreColumnSetCode, semantic.ReuseFromCorpus); rmErr != nil {
	...
	if upErr := manager.semantic.Reindex(ctx, root, fileResult.Chunks, semantic.RemovePaths([]string{relativePath}), nil, nil, semantic.StoreColumnSetCode, semantic.ReuseFromCorpus); upErr != nil {
