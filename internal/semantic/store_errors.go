...
// wrapStoreError logs and classifies a milvus call error from the write, index,
// or read/check paths. operation names the failing call (for example "create
// Milvus collection <name>"). A transport-unavailable store outage becomes a
// ClassMilvusUnavailable adapter error, which the daemon treats as a
// shared-infrastructure outage that keeps the codebase resumable behind the
// health banner rather than marking it failed. Every other error is wrapped
// plainly so it keeps its existing classification (a real per-collection fault).
// The cause is appended with %w so [errors.Is] and [errors.As] still reach it. It
// logs the failure once so call sites do not, and a nil err returns nil.
