...
func TestEngineCloseSerializesWithToolAndIndex(t *testing.T) {
	repositoryPath := t.TempDir()
	writeTestRepository(t, repositoryPath, "closerace")

	cacheDir := t.TempDir()
	t.Setenv("CBM_CACHE_DIR", cacheDir)
	engine, errorMessage := Open("closerace", cacheDir)
	if errorMessage != nil {
		t.Fatal(errorMessage)
	}

	const workerCount = 8
	errs := make(chan error, workerCount*3)
	var waitGroup sync.WaitGroup
	for i := 0; i < workerCount; i++ {
		waitGroup.Add(3)
		go func() {
			defer waitGroup.Done()
			if err := engine.Index(context.Background(), repositoryPath, "fast"); err != nil && !strings.Contains(err.Error(), "cbm engine is closed") {
				errs <- err
			}
		}()
		go func() {
			defer waitGroup.Done()
			_, err := engine.Tool("query_graph", queryGraphFunctionsJSON)
			if err != nil && !strings.Contains(err.Error(), "cbm engine is closed") {
				errs <- err
			}
		}()
		go func() {
...
func TestEngineToolRejectsIndexRepository(t *testing.T) {
	engine, errorMessage := Open("rejectindex", t.TempDir())
	if errorMessage != nil {
		t.Fatal(errorMessage)
	}
	defer engine.Close()

	if _, errorMessage = engine.Tool("index_repository", `{}`); errorMessage == nil {
		t.Fatal("Tool accepted index_repository, want error")
	}
}
