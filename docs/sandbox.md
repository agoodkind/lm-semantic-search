...
The sandbox roots every path the daemon reads or writes inside one directory, apart from the shared model cache described below, and that includes `mcp-sync.flock`. That lock is the reason the context root moves: a daemon that relocates only its state still competes for the operator's lock, and a daemon holding it blocks every indexing job in the daemon that wants it.
