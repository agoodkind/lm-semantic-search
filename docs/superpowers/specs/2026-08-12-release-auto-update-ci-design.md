...
The probe uses Go. It selects releases through the GitHub API, proxies update API requests with the workflow token, extracts the complete old release archive, isolates all runtime paths, and reports daemon and updater diagnostics on failure.
